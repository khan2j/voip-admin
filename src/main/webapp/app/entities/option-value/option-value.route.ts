...
export class OptionValueResolve implements Resolve<IOptionValue> {
  constructor(private service: OptionValueService, private router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<IOptionValue> | Observable<never> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        flatMap((optionValue: HttpResponse<OptionValue>) => {
          if (optionValue.body) {
...
export const optionValueRoute: Routes = [
  {
    ...
    data: {
      ...
      defaultSort: 'id,asc',
      pageTitle: 'voipadminApp.optionValue.home.title',
    },
