...
describe('Setting e2e test', () => {
  ...
  it('should load Settings', async () => {
    ...
    expect(await settingComponentsPage.getTitle()).to.eq('voipadminApp.setting.home.title');
  ...
  it('should load create Setting page', async () => {
    ...
    expect(await settingUpdatePage.getPageTitle()).to.eq('voipadminApp.setting.home.createOrEditLabel');
  ...
  it('should create and save Settings', async () => {
    ...
    await promise.all([
      settingUpdatePage.setTextValueInput('textValue'),
      settingUpdatePage.optionSelectLastOption(),
      // settingUpdatePage.selectedValuesSelectLastOption(),
      settingUpdatePage.deviceSelectLastOption(),
    ]);

    expect(await settingUpdatePage.getTextValueInput()).to.eq('textValue', 'Expected TextValue value to be equals to textValue');
  ...
  it('should delete last Setting', async () => {
    ...
    expect(await settingDeleteDialog.getDialogTitle()).to.eq('voipadminApp.setting.delete.question');
